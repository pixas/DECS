...
class LengthAdvRewardManager:
    """The reward manager.
    """

    def __init__(self, tokenizer, num_examine, reward_config=None, compute_score=None, reward_fn_key='data_source',max_resp_len=-1, format_score=0, eval=False, gamma=2e-5) -> None:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            ...
            if reward > 0:
                # compute length reward , length_reward_tensor[i, t] = t/valid_response_length, t = 0: valid_response_length -1 
                length_reward_tensor[i, :valid_response_length] = 1 / (valid_response_length -  torch.arange(0, valid_response_length, dtype=torch.float32))
                length_reward_tensor[i, valid_response_length:] = 1
                

            else:
                length_reward_tensor[i, :valid_response_length] = 0
