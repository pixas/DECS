...
DASHBOARD_AGENT_LISTEN_PORT=53002
srun --nodes=1 --ntasks=1 -w "$head_node" \
    ray start --head \
        ...
        --object-store-memory="${MEMORY_SIZE}" \
        --min-worker-port 10901 \
        --temp-dir /mnt/petrelfs/jiangshuyang/tmp/ray \
        --dashboard-agent-listen-port "${DASHBOARD_AGENT_LISTEN_PORT}" \
        --max-worker-port 19999 --verbose &
