...
# self.retriever = AutoModelForSequenceClassification.from_pretrained(retriever_path)
retriever = AutoModel.from_pretrained(args.retriever_model).cuda()
# from sentence_transformers import SentenceTransformer
# retriever = SentenceTransformer(args.retriever_model).cuda() 
...
def LLM_carry(prompt):
    ...
    with torch.no_grad():
        ...
        # embeds = retriever(**encoded).last_hidden_state[:, 0, :]
    return embeds
...
async def retrieve_endpoint(request: Request):
    ...
    query = json_post_list.get("queries")
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            embeds = await loop.run_in_executor(executor, LLM_carry, query)
