...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, beta=0.1):
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        if getattr(self.config.data, "weighted", False):
            train_dataloader_generator = torch.Generator()
            train_dataloader_generator.manual_seed(self.config.data.get('seed', 1))
            # check self.train_dataset.acc is not None
            if self.train_dataset.ref_acc is not None:
            # use ref_acc as the weight for WeightedRandomSampler
                weights = 1 - self.train_dataset.ref_acc
                sampler = WeightedRandomSampler(weights=weights, num_samples=len(self.train_dataset), replacement=False, generator=train_dataloader_generator)
            else:
                print("Warning: The data source do not contain a acc field to serve as weights. Back to random sampler")
                sampler = RandomSampler(data_source=self.train_dataset, generator=train_dataloader_generator)
        else:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        batch.batch['original_token_level_score'] = torch.tensor(reward_extra_infos_dict['original_score']).to(reward_tensor).unsqueeze(-1) if 'original_score' in reward_extra_infos_dict else reward_tensor
                    ...
                    if is_last_step or (self.config.trainer.save_freq > 0 and \
                            (self.global_steps % self.config.trainer.save_freq == 0 and self.global_steps > 1)):
                ...
                if is_last_step:
                    ...
                    send_feishu_message(last_val_metrics)
                    progress_bar.close()
                    return 
