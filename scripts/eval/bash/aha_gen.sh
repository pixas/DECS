...
for DATASET in "${DATASETS[@]}"; do 
    LOG_FILE=${OUTPUT_ROOT}/${DATASET}/${model_name}_${GROUP_NAME}/aha_infer.log

    CONFIG_PATH=${OUTPUT_ROOT}/${DATASET}/${model_name}_${GROUP_NAME}/result.json
    ...
    if  [[ $CHUNK_NUM != 1 ]]; then 
        for i in $(seq 0 $((CHUNK_NUM-1))); do
            OUTPUT_PATH=${OUTPUT_ROOT}/${DATASET}/${model_name}_${GROUP_NAME}/chunk_${i}
            mkdir -p $OUTPUT_PATH
            LOG_FILE=${OUTPUT_PATH}/aha_infer.log
            sbatch -o $LOG_FILE ./scripts/eval/slurm/aha_gen.sh \
                $CONFIG_PATH "--chunk_num $CHUNK_NUM --chunk_idx $i" ${@:4}
            sleep 2
        done
    else
        LOG_FILE=${OUTPUT_ROOT}/${DATASET}/${model_name}_${GROUP_NAME}/aha_infer.log
