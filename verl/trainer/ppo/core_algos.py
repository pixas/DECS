    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                cur_std = torch.std(torch.tensor([id2score[idx]]))
                id2std[idx] = cur_std
                # if torch.equal(cur_std, torch.tensor(0.0)):
                #     id2std[idx] = torch.tensor(1.0)
                # else:
                #     id2std[idx] = cur_std
            else:
...
def compute_reinforce_pp_length_reward_outcome_adv(token_level_rewards: torch.Tensor,
                                                   response_mask: torch.Tensor,
                                                    length_reward: torch.Tensor,
                                                    index: np.ndarray,
                                                    gamma: float = 2e-5,
                                                    epsilon: float = 1e-6,
                                                    reward_config=None):
    token_level_scores = token_level_rewards.sum(-1)
    if reward_config.no_zero_adv:
    ...
    token_length_scores = length_reward 
    final_token_rewards = token_correctness_scores + token_length_scores * gamma * (1 / response_mask.shape[1])  # normalize by response length
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                # id2std[idx] = current_batch_token_rewards_sigma if not torch.equal(current_batch_token_rewards_sigma, torch.tensor(0.0)) else torch.tensor(1.0)
        for i in range(bsz):
...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
def compute_grpo_length_reward_outcome_advantage(token_level_rewards: torch.Tensor,
                                   response_mask: torch.Tensor,
                                   length_reward: torch.Tensor,
                                   index: np.ndarray,
                                   gamma: float = 2e-5,
    ...
    if reward_config.no_zero_adv:
        ...
        token_correctness_scores = token_level_scores.unsqueeze(-1) * response_mask
    token_length_scores = length_reward 
    final_token_rewards = token_correctness_scores + token_length_scores * gamma   # normalize by response length
    final_token_advantages = torch.zeros_like(final_token_rewards)
    # we should separate to compute the baseline 
...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
def compute_grpo_length_outcome_advantage(token_level_rewards: torch.Tensor,
                                   response_mask: torch.Tensor,
                                   length_reward: torch.Tensor,
                                   index: np.ndarray,
                                   gamma: float = 2e-5,
    ...
    base_advantages, _ = compute_reinforce_plus_plus_baseline_outcome_advantage(token_level_rewards, response_mask, index)
    
    scores = token_level_rewards.sum(dim=-1)
    # advantages = torch.zeros_like(scores)
    # returns = torch.zeros_like(scores)
    length_advantage = torch.zeros_like(token_level_rewards)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            # id2score[index[i]].append(scores[i])
            if scores[i] > 0:
                correct_buffer.append(length_reward[i])
                correct_response_mask.append(response_mask[i])  
                # id2length_reward[index[i]].append(length_reward[i])
                # id2response_mask[index[i]].append(response_mask[i])
        temp_length_reward = torch.stack(correct_buffer, dim=0)
        temp_length_response_mask = torch.stack(correct_response_mask, dim=0)
        temp_length_reward = temp_length_reward * temp_length_response_mask
        
        temp_length_reward_mean = verl_F.masked_mean(temp_length_reward, temp_length_response_mask, axis=0)
                
        for i in range(bsz):
            if scores[i] > 0:
                length_advantage[i] = length_reward[i] - temp_length_reward_mean
        length_advantage = length_advantage * response_mask

        
        advantages = (base_advantages + length_advantage * gamma) * response_mask
        


    return advantages, base_advantages
...
def compute_reinforce_plus_plus_outcome_process_advantage(token_level_rewards: torch.Tensor, process_token_level_rewards: torch.Tensor, response_mask: torch.Tensor, lam: torch.Tensor,
                                                  gamma: torch.Tensor):
    """
    Compute advantage for REINFORCE++. 
    This implementation is based on the paper: https://arxiv.org/abs/2501.03262
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(token_level_rewards.shape[1])):
            running_return = token_level_rewards[:, t] + gamma * running_return
            returns[:, t] = running_return + process_token_level_rewards[:, t] * response_mask[:, t] * lam
            # Reset after EOS
            running_return = running_return * response_mask[:, t]
