...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        model_state_dict = client.load_checkpoint(local_model_path, weights_only=False)
        optimizer_state_dict = client.load_checkpoint(local_optim_path, weights_only=False)
        extra_state_dict = client.load_checkpoint(local_extra_state_path, weights_only=False)
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                print(f'[rank-{self.rank}]: Saving model to {model_path}')
                print(f'[rank-{self.rank}]: Saving checkpoint to {optim_path}')
                print(f'[rank-{self.rank}]: Saving extra_state to {extra_path}')
                client.save_checkpoint(model_state_dict, model_path)
                client.save_checkpoint(optimizer_state_dict, optim_path)  # TODO: address optimizer is None
                client.save_checkpoint(extra_state_dict, extra_path)
        ...
        if max_ckpt_to_keep and isinstance(max_ckpt_to_keep, int) and max_ckpt_to_keep > 0 and len(
            ...
            paths_to_remove = self.previous_saved_paths[:keep_start]
            if paths_to_remove:  # Only attempt removal if there are paths to remove
