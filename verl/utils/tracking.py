...
@dataclasses.dataclass  
class TrainGenerationsLogger(ValidationGenerationsLogger):
    def log_generations_to_wandb(self, samples, step):
        """Log samples to wandb as a table"""
        import wandb

        # Create column names for all samples
        columns = ["step"] + sum([[f"input_{i+1}", f"output_{i+1}", f"manual_{i+1}", f"score_{i+1}"] for i in range(len(samples))], [])
