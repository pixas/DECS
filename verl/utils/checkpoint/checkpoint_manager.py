...
class BaseCheckpointManager:
    ...
    def remove_previous_save_local_path(self, path):
        if isinstance(path, str):
            path = [path]
        for p in path:
            abs_path = client.abspath(p)
            # abs_path = os.path.abspath(p)
            print(f'Checkpoint manager remove previous save local path: {abs_path}')
            # if not os.path.exists(abs_path):
            if not client.exists(abs_path):
                continue
            # shutil.rmtree(abs_path, ignore_errors=True)
            client.remove(abs_path)
...
def find_latest_ckpt_path(path, directory_format="global_step_{}"):
    ...
    if not client.exists(tracker_file):
        print("Checkpoint tracker file does not exist: %s", tracker_file, flush=True)
        return None

    iteration = int(client.read_txt(tracker_file))
    # with open(tracker_file, "rb") as f:
    #     iteration = int(f.read().decode())
    ckpt_path = os.path.join(path, directory_format.format(iteration))
    # if not os.path.exists(ckpt_path):
    if not client.exists(ckpt_path):
        print(f"Checkpoint does not exist: {ckpt_path}", flush=True)
        return None

    print("Found checkpoint: %s", ckpt_path, flush=True)
    return ckpt_path
...
def get_checkpoint_tracker_filename(root_path: str):
    ...
    """
    if "s3://" in root_path:
        return root_path + "/latest_checkpointed_iteration.txt" if not root_path.endswith("/") else "latest_checkpointed_iteration.txt"
    else:
