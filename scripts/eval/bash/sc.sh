...
# DATASETS=(aime2024 aime2025 amc23 math olympiad_bench gpqa_diamond)
DATASETS=(math)
CHUNK_NUM=${2:-1}
SC_SIZE=${3:-16}
TP_SIZE=${4:-1}
...
for DATASET in "${DATASETS[@]}"; do 
    LOG_FILE=${OUTPUT_ROOT}/${DATASET}/${model_name}_sc${SC_SIZE}/infer.log

    mkdir -p ${OUTPUT_ROOT}/${DATASET}/${model_name}_sc${SC_SIZE}
    ...
    if  [[ $CHUNK_NUM != 1 ]]; then 
        for i in $(seq 0 $((CHUNK_NUM-1))); do
            ...
            sbatch -o $LOG_FILE --gres=gpu:${TP_SIZE} ./scripts/eval/slurm/sc.sh \
                ${DATA_ROOT}/${DATASET}/test.parquet \
                ${OUTPUT_PATH} \
                ${model_name_or_path} ${SC_SIZE} "--tp_size ${TP_SIZE}" ${@:5}
            sleep 2
        done
    else
        LOG_FILE=${OUTPUT_ROOT}/${DATASET}/${model_name}_sc${SC_SIZE}/infer.log
        # if [ ! -f "${OUTPUT_ROOT}/${DATASET}/${model_name}_sc${SC_SIZE}/cache.jsonl" ]; then
            job_id=$(sbatch -o $LOG_FILE --gres=gpu:${TP_SIZE} ./scripts/eval/slurm/sc.sh \
                ${DATA_ROOT}/${DATASET}/test.parquet \
                ${OUTPUT_ROOT}/${DATASET}/${model_name}_sc${SC_SIZE} \
                ${model_name_or_path} ${SC_SIZE} "--tp_size ${TP_SIZE}" "${@:5}" | awk '{print $4}')
