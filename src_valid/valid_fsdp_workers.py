...
class ValidActorRolloutRefWorker(Worker):
    """
    ...
    def init_model(self):
        from .valid_dp_actor import DataParallelPPOActor
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        data.meta_info['micro_batch_size'] = self.config.rollout.log_prob_micro_batch_size_per_gpu if data.meta_info.get('micro_batch_size', None) is None else data.meta_info['micro_batch_size']
        data.meta_info['max_token_len'] = self.config.rollout.log_prob_max_token_len_per_gpu if data.meta_info.get('max_token_len', None) is None else data.meta_info['max_token_len']
        data.meta_info['use_dynamic_bsz'] = self.config.rollout.log_prob_use_dynamic_bsz if data.meta_info.get('use_dynamic_bsz', None) is None else data.meta_info['use_dynamic_bsz']
    ...
    def compute_log_prob_entropy(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            # output = self.actor.compute_log_prob(data=data)  # (bs, response_length)
            result = self.actor.compute_log_prob_entropy(data=data)
            output = DataProto.from_dict(tensors={'old_log_probs': result.batch['log_prob'], 'old_entropy': result.batch['entropy']},
                                         meta_info={'temperature': self.config.rollout.temperature})
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        log_gpu_memory_usage('After compute_log_prob_entropy', logger=logger)
        return output
