...
NUM_GPUS=$(srun --nodes=1 --ntasks=1 -w "$head_node" nvidia-smi -L | wc -l)
...
srun --nodes=1 --ntasks=1 -w "$head_node" \
        ...
        --num-cpus "${SLURM_CPUS_PER_TASK}" --num-gpus "${NUM_GPUS}" --block --object-store-memory="${MEMORY_SIZE}" --dashboard-host=0.0.0.0 --min-worker-port 10101 --max-worker-port 19999 \
        --temp-dir="/tmp/jiangshuyang" &

sleep 10

HYDRA_FULL_ERROR=1
RAY_ADDRESS="http://${head_node_ip}:${DASHBOARD_PORT}" ray job submit -- python3 -u -m src.reflect_main_ppo \
