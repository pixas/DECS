...
def main(config):
    ...
    save_path = os.path.join(os.path.dirname(local_path), 'result.json')
    if config.data.path.endswith('.parquet'):
        dataset = pd.read_parquet(local_path)
    elif config.data.path.endswith('.json'):
        dataset = pd.read_json(local_path, lines=False)
    ...
    compute_score = compute_score if compute_score is not None else _default_compute_score
    # Create remote tasks
    remote_tasks = [
    ...
    save_contents = {
        "score": {
            "acc": metric_dict[f'test_score/{data_source}'],
        },
        "count": total
    }
    print(metric_dict)
    with open(save_path, 'w') as f:
        json.dump(save_contents, f, indent=4, ensure_ascii=False)
